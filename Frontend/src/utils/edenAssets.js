...
export function preloadSounds(soundMap = {}) {
  Object.values(soundMap).forEach((sound) => preloadSound(sound));
}
...
export function playSound(src, volume = 0.45) {
  ...
  try {
    ...
    audio.play().catch((error) => {
      console.warn("Audio blocked:", error?.message || error);
    });
...
export function getThemeLogo() {
  return EDEN_ASSETS.logos.ucnmvc;
}

export function verifySoundFile(src) {
  return fetch(src, { method: "HEAD" })
    .then((response) => response.ok)
    .catch(() => false);
}

export default EDEN_ASSETS;
